...
-- Настройки наклона для виса
local hangingTiltMultiplier = 0.3  -- Уменьшенный наклон при висе (30% от обычного)
local hangingLerpSpeed = 0.03      -- Более плавная интерполяция при висе
...
bobbing = game:GetService("RunService").RenderStepped:Connect(function(deltaTime)
	...
	-- Пропускаем если меню настроек открыто
	local settingsOpen = Player:FindFirstChild("SettingsMenuOpen")
	if settingsOpen and settingsOpen.Value then
		return
	end
	...
	-- Пропускаем если панель персонажа открыта (камера управляется CharacterPanel)
	local characterPanelOpen = Player:FindFirstChild("CharacterPanelOpen")
	if characterPanelOpen and characterPanelOpen.Value then
		return
	end
	...
	-- Проверяем состояние виса
	local isHanging = LedgeGrabConfig.IsHanging
	
	if isHanging then
		-- При висе: только боковой наклон (A/D), уменьшенный и плавный
		local sideVelocity = Humanoid.RootPart and Humanoid.RootPart.Velocity or Vector3.new()
		local lateralX = Camera.CFrame:VectorToObjectSpace(sideVelocity).X
		
		-- Только боковое движение влияет на наклон, уменьшенный множитель
		val2 = math.clamp(lerp(val2, -lateralX * 0.01 * hangingTiltMultiplier, hangingLerpSpeed * deltaTime), -0.04, 0.04)
		func3 = lerp(func3, math.clamp(UserInputService:GetMouseDelta().X, -1, 1), 0.1 * deltaTime)
	else
		-- Обычное поведение
